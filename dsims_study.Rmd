...
This case study shows you how to use the R package **dsims** [@dsimspkg] to compare the performance of different survey designs.  We will replicate the analyses of Sections 3.5.2 and 11.1.1 of [@Buckland2015b].
...
Examine the other files and folders in the "dsims_study" folder. There are three files starting with the name "Region" and ending with .dbf, .shp and .shx, these files make up the shapefile for the survey region. The `density.surface.robj` file is the density surface for the survey region. The *Survey Transects* folder contains files wth the name "subj_des" describing the shapefile to be used for the subjective design. The *Results* folder contains the results from 999 replications as this can take a good few hours to run. Select the directory that contains these files.

# Creating a new simulation

## Creating a region object

First we reate the region object using the region shapefile. As there are no strata in this example, you just need to provide a name for your survey region and specify the units (metres, m). 
...
There is no way to specifically tell dsims (in fact the design package dssd) that the design is a subjective design so instead we will define it as a random line transect design. Later when we run the simulation we will point the simulation to a single shapefile containing the transects that we wish to use. 
...
Note that now we need to tell it to use our subjective design transects for the simulation (otherwise it will generate randomly placed lines).
...
```{r zigzag, echo=FALSE, eval=FALSE, warning=FALSE}
